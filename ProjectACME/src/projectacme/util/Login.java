...
public class Login {
    public static boolean validationUser(String document, String password){
        ...
        if (accessSubject != null && (accessSubject.getRole().name().equals("sudo") || accessSubject.getRole().name().equals("officer") || accessSubject.getRole().name().equals("manager") || accessSubject.getRole().name().equals("securityGuard"))){
        return accessSubject.getPassword().equals(password);
        }
        return false;
    }
}
